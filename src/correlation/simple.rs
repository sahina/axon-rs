...
impl SimpleCorrelationProvider {
    /// Constructs a `SimpleCorrelationProvider` with given list of keys. If any of these keys in
    /// `header_names` exist in message's `MetaData`, `correlation_for` method will return
    /// `MetaData` as correlation data.
